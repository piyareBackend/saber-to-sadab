...
class ShapePaletteDialog extends StatefulWidget {
  const new({super.key, required this.onSelected});
  final ValueChanged<CustomShapeDefinition> onSelected;
  @override
  State<ShapePaletteDialog> createState() => _ShapePaletteDialogState();
}

class _ShapePaletteDialogState extends State<ShapePaletteDialog> {
  ShapeFill fill = ShapeFill.transparent;
  ShapeStrokeStyle stroke = ShapeStrokeStyle.solid;
  double weight = 3;
  IconData iconFor(ShapeKind kind) => switch (kind) {
    ShapeKind.line => Icons.horizontal_rule,
    ShapeKind.arrow => Icons.arrow_forward,
    ShapeKind.doubleArrow => Icons.swap_horiz,
    ShapeKind.rectangle => Icons.crop_square,
    ShapeKind.roundedRect => Icons.rounded_corner,
    ShapeKind.ellipse => Icons.circle_outlined,
    ShapeKind.axes => Icons.center_focus_strong,
      ...
      child: Column(
        ...
        children: [
          GridView.builder(
            ...
            itemCount: CustomShapeRegistry.defaults.length,
            itemBuilder: (_, i) {
          ...
          SegmentedButton<ShapeFill>(
            segments: const [
              ButtonSegment(value: ShapeFill.transparent, label: Text('Clear')),
              ButtonSegment(value: ShapeFill.translucent, label: Text('Glass')),
              ButtonSegment(value: ShapeFill.solid, label: Text('Solid')),
            ],
            selected: {fill},
            onSelectionChanged: (v) => setState(() => fill = v.first),
          ),
          SegmentedButton<ShapeStrokeStyle>(
            ...
            selected: {stroke},
            onSelectionChanged: (v) => setState(() => stroke = v.first),
          ),
          Row(
            children: [
              const Text('Weight'),
